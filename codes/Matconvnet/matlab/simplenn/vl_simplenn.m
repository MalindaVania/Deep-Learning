...
for i=1:n
    ...
    switch l.type
        case 'conv'
            %        l=gpuArray(l);
            % x=gather(res(i).x);
            % filters=gpuArray(l.filters);
            % biases=gpuArray(l.biases);
            % l.filters=filters;
            % l.biases=biases;
        ...
        case 'softmaxloss3D'
            res(i+1).x = vl_nnsoftmaxloss3D(res(i).x, l.class) ;
        case 'relu'
            res(i+1).x = vl_nnrelu(res(i).x) ;
        case 'conv3D'
            res(i+1).x = mex_conv3d(res(i).x,l.filters,l.biases) ;
        case 'pool3D'
            [a b] = mex_maxpool3d(res(i).x,'pool',l.pool) ;
            res(i+1).x=a;
            ind{v}=b;
            v=v+1;
            clearvars a b
...
if doder
    ...
    for i=n:-1:1
        ...
        switch l.type
            ...
            case 'conv3D'
                %            dzdx=gather(res(i+1).dzdx);
                % DZDY=ones(size(res(i+1).x), 'like', (res(i+1).x));
                % DZDY=single(gpuArray(res(i+1).dzdx));
                % DZDY=reshape(DZDY,[size(res(i+1).x)]);
                [res(i).dzdx, res(i).dzdw{1}, res(i).dzdw{2}] = ...
                    mex_conv3d(res(i).x, l.filters, l.biases, ...
                    res(i+1).dzdx);
            case 'pool3D'
                dzdy=rand(size(res(i).x),'like',res(i).x);
                res(i).dzdx = mex_maxpool3d(dzdy,ind{v},size(res(i+1).x),'pool',l.pool);
                v=v-1;
            case 'pool'
            ...
            case 'softmaxloss3D'
                res(i).dzdx = vl_nnsoftmaxloss3D(res(i).x, l.class, res(i+1).dzdx) ;
            case 'relu'
