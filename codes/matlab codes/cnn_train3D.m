function [net, info] = cnn_train3D(net, imdb, getBatch, varargin)
...
for i=1:numel(net.layers)
    %% I am adding the below lines (36-37) to input the upsample rate into network
    %I am adding the below lines (41-58) for 3D convolution
 
    if strcmp(net.layers{i}.type,'convt'), 
net.layers{i}.upsample= net.layers{i}.upsample;
      end
      if strcmp(net.layers{i}.type,'conv3D'),
...
if opts.useGpu
  ...
  for i=1:numel(net.layers)
    if ~strcmp(net.layers{i}.type,'conv3D'), continue; end
...
for epoch=1:opts.numEpochs
  ...
  if prevLr ~= lr
    ...
    for l=1:numel(net.layers)
      if ~strcmp(net.layers{l}.type, 'conv3D'), continue ; end
...
% -------------------------------------------------------------------------
% predictions = gather(res(end-1).x) ; %% changing it to res(end)
...
switch opts.errorType
  case 'multiclass'
    [~,predictions] = sort(predictions, 4, 'descend') ;
    error = ~bsxfun(@eq, predictions, reshape(labels,1, 1, 1,1, [])) ;
    info.error(end) = info.error(end) +....
      sum(sum(sum(error(:,:,:,1,:))))/n ;
%     info.topFiveError(end) = info.topFiveError(end) + ...
%       sum(sum(sum(min(error(:,:,:,:),[],3))))/n ; %%% I changed this
  case 'binary'
