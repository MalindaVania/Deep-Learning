function Y = vl_nnsoftmaxloss3D(X,c,dzdy)
...
if size(c,1) == 1 & size(c,2) == 1 & size(c,3)==1
...
sz_ = [size(c,1) size(c,2) size(c,3) size(c,4) size(c,5)] ;
assert(isequal(sz_, [sz(1) sz(2) sz(3) sz_(4) size(c,5)])) ;
assert(sz_(4)==1 | sz_(4)==2) ;
...
c_ = 0:numel(c)-1 ;
c_ = 1 + ...
  mod(c_, sz(1)*sz(2)*sz(3)) + ...
  (sz(1)*sz(2)*sz(3)) * max(c(:), 0)' + ...
  (sz(1)*sz(2)*sz(3)*sz(4)) * floor(c_/(sz(1)*sz(2)*sz(3))) ;
...
%n = sz(1)*sz(2) ;
if nargin <= 2
  t = Xmax + log(sum(ex,4)) - reshape(X(c_), [sz(1:3) 1 sz(5)]) ;
  Y = sum(sum(sum(sum(mass .* t,1),2),3),5) ;
else
